...
Това е същото като `data`, но благодарение на следното ограничение:
* `newtype` типовете винаги имат точно един конструктор с точно едно поле
Haskell ни гарантира че новият ни тип (`RacketProgram`) се държи по точно същия
начин като това което съдържа (`[RacketExpr]`) от гледна точка на памет.

Това е различно от `data` типовете, които задължително съдържат поне едно тагче за
кой от конструкторите има стойността, както и допълнителен указател който да сочи
към данните в даден конструктор на `data`.

С други думи `newtype` ползваме когато не искаме да хабим повече ресурси,
но искаме типовата система да ни дава някаква полезна за нас като програмисти информация.
Много удобни са когато пишем библиотека и искаме тя да има "безопасен" интърфейс,
което постигаме като скрием конструктора на `newtype`-а.
...
Има булева констант `solvingCompiler`, която в момента е `False`.
...
За да заобиколим това, имаме някои опции. Един такъв би бил, ако при превеждане бихме имали
недифинирани имена (т.е. има свободни променливи), вместо израз, да генерираме `define`
декларация, която да дефинира функция, взимаща тези променливи като аргументи.
...
Имплементирайте главната функция грижеща за компилация.
...
"Поддървета" ще наричам всички изрази които се съдържат в даден друг, рекурсивно.
...
Целта на оптимизацията е да махнем всички поддървета (освен `Val`-ове), които можем да оценим без да знаем
...
че условието на `If` е сметка която ще **успее**.

Заради това, няма да го имплементираме (както и защото ме мързи да си оправям тестовете
да взимат това предвид.).

По-конкретно, тук може да нямаме нямаме `"x"` в контекста, когато се опитваме да оценим изрза, съответно това би трябвало да се оцени до `Nothing`.
...
#### Тестове
За да пускате тестовете за `partialEval`, промене
